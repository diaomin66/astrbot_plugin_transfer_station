...
class QuotaSnapshot:
    ...
    def amount_to_quota(self, value: Decimal | str | int) -> int:
        ...
        quota = int(raw.quantize(Decimal(1), rounding=ROUND_HALF_UP))
        if quota <= 0:
...
class NewApiClient:
    ...
    async def _request_json(
        ...
        *,
        ...
        try:
            ...
            kind = "ambiguous" if write and response.status_code >= 500 else "clear"
            raise NewApiError(
                "New API 返回了无法解析的响应",
                kind=kind,
        ...
        if not isinstance(payload, dict):
            raise NewApiError("New API 返回结构无效")
    ...
    async def add_quota(self, user_id: int, raw_quota: int) -> None:
        if int(user_id) <= 0 or int(raw_quota) <= 0:
