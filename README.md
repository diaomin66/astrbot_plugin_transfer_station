...
`v1.4.0` 新增独立 Plugin Page；`v1.4.1` 增加旧版 New API 认证兼容与错误诊断；`v1.4.2` 将普通中奖者领奖从管理员抽奖命令中抽离；`v1.4.3` 同步抽离普通用户补偿领取指令：
...
- 普通中奖者使用独立的 `/领奖 <New API用户ID>` 和 `/确认领奖`，不再经过 `/抽奖` 管理命令的 `ADMIN` 权限检查。
- 普通用户使用独立的 `/领取补偿 <New API用户ID>` 和 `/确认补偿`，不再经过 `/补偿` 管理命令的 `ADMIN` 权限检查。
...
安装或更新后重启 AstrBot，并确认插件版本为 `v1.4.3`。
